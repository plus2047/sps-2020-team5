...
def notes2midi(input_file, output_dir):
    parser = argparse.ArgumentParser(description=__doc__)

    parser.set_defaults(func=main)
    #parser.set_defaults(input_file=input_file, type=argparse.FileType('rb'), metavar='FILE')
    parser.set_defaults(output_dir=output_dir, type=str, metavar='OUTPUTDIR')
    parser.set_defaults(instrument='', type=str)
    parser.set_defaults(drums=False)
    parser.set_defaults(program=None, type=int)
    parser.set_defaults(stretch="60:115", type=str)
    parser.set_defaults(tempo=None, type=float)
    parser.set_defaults(time_signature=(4, 4), type=lambda s: tuple(int(x) for x in s.split('/')))
    parser.set_defaults(resolution=480, type=int)
    parser.set_defaults(range=None, type=lambda r: [None if x == '' else int(x)
                                                   for x in r.split(':')])
    parser.set_defaults(group_by_name=False)
    parser.set_defaults(time_unit=4, type=float)
    args = parser.parse_args()
