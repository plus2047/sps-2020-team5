...
class cyclegan(object):
    ...
    def serve(self, args, input_dir, output_dir):
        ...
        sample_files = sorted(glob(input_dir + "/*.npy"))
        ...
        for f in sample_files:
            print('Processing midi: ', f)
            stem = Path(f).stem
            sample_npy = np.load(f) * 1.
            sample_npy_re = sample_npy.reshape(1, sample_npy.shape[0], sample_npy.shape[1], 1)
            midi_path_origin = os.path.join(output_dir, '{}_origin.mid'.format(stem))
            midi_path_transfer = os.path.join(output_dir, '{}_transfer.mid'.format(stem))
            midi_path_cycle = os.path.join(output_dir, '{}_cycle.mid'.format(stem))
            origin_midi, fake_midi, fake_midi_cycle = self.sess.run([out_origin, out_var, out_var_cycle],
                                                                    feed_dict={in_var: sample_npy_re})
            save_midis(origin_midi, midi_path_origin)
            save_midis(fake_midi, midi_path_transfer)
